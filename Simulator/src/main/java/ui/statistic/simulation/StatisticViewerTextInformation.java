...
public class StatisticViewerTextInformation extends StatisticViewerText {
	...
	private void buildSystemInformation() {
		...
		if (statistic.runTime>0) {
			...
			if (statistic.kundenGlobal.anrufe>0) addLine(Language.tr("SimStatistic.SystemData.TimePerCall")+t+": "+NumberTools.formatNumber(1000*(double)statistic.runTime*statistic.runThreads/statistic.kundenGlobal.anrufe,1)+" µs");
			if (statistic.simEvents>0) addLine(Language.tr("SimStatistic.SystemData.TimePerEvent")+t+": "+NumberTools.formatNumber(1000*(double)statistic.runTime*statistic.runThreads/statistic.simEvents,2)+" µs");
			endParagraph();
		}
