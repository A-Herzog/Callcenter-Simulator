...
public final class CallCancelEvent extends Event {
	...
	public static void logGiveUp(final SimulationData data, final CallerRecord callerRecord, final long time, final boolean retry, final boolean endOfDay) {
		...
		if (cancelTime<0) cancelTime=86400+cancelTime; /* Abbruch war eigentlich erst am nächsten Tag */
		assert(cancelTime>=0);
		...
		if (endOfDay) {
			...
			/* anrufeAbbruchzeitSum wird in ComplexSimData.finalTerminateCleanUp aus den anrufeAbbruchzeitSumProInterval-Daten zusammengezählt */
	...
	/**
	 * Erfasst den Abbruch eines Kunden des sich momentan außerhalb des Systems befindet.
	 * @param data	Simulationsdatenobjekt
	 * @param callerRecord	Kundendatensatz
	 * @param time	Zeitpunkt des Abbruchs
	 * @param plannedRetryTime	Geplante Wiederholzeit am nächsten Tag (einer berbundenen Simulation)
	...
	public static void logGiveUpWhileExtern(final SimulationData data, final CallerRecord callerRecord, final long time, final long plannedRetryTime, final boolean endOfDay) {
		...
		if (endOfDay) {
			...
			/* kundenAbbruchzeitSum wird in ComplexSimData.finalTerminateCleanUp aus den kundenAbbruchzeitSumProInterval-Daten zusammengezählt */
	...
	public void run(final SimData data) {
		...
		if (callerRecord.agentAssignedStartEvent!=null) {
			...
			/* Ereignis, welche den Gesprächsbeginn darstellt, löschen */
